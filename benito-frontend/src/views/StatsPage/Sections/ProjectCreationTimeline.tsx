...
const ProjectCreationTimeline = (props: ProjectCreationTimelineProps) => {
  ...
  const [maxCategoriesSelected, setMaxCategoriesSelected] = useState<boolean>(
    false
  );
  ...
  const results = withProjectCreationTimeline("", (r) => {
    ...
    setCategoriesData(
      r.map((result: ProjectCreationTimelineType) =>
        convertToTimelineData(result)
      )
    );
  });
  ...
  return (
    <div>
      <div
        ...
        style={{ paddingTop: "20px", color: color }}
      >
      ...
      {selectedCategories.map((s, idx) => (
        <div
          ...
          onClick={() => {
            let scids: Array<string> = selectedCategories.map((sc) =>
              convertCategoryNameToId(sc)
            );
            if (scids) {
            ...
            setSelectedCategories(
              scids.map((sc) => convertCategoryIdToName(sc))
            );

            if (!invalidYears) {
              updateProjectCreationTimeline(scids, startYear, endYear).then(
                (r) => {
                  setLabels(r.data[0].quantities.map((tl) => tl.year));
                  setCategoriesData(
                    r.data.map((result: ProjectCreationTimelineType) =>
                      convertToTimelineData(result)
                    )
                  );
                }
      ...
      >
        <GridItem xs={12} sm={12} md={4} lg={4}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} fullWidth label="Categoría" />
            )}
          />
        </GridItem>
